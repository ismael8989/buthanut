...
class AuthController extends Controller
{
    ...
    public function register(Request $request) {
        ...
        if (!preg_match('/^0(5|6|7)\d{8}$/', $validated['tel'])) {
            return response()->json(["error" => "The number you provided isn't a valid Moroccan telephone number"], 422);
        }

        $tel = DB::table("tel_numbers")->where('tel', $request->tel)->first();

        if(!$tel || !$tel->verified) {
            return response()->json(['error' => "Your phone number isn't verified yet"], 400);
        }
